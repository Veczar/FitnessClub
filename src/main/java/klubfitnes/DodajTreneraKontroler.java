...
public class DodajTreneraKontroler {
    ...
    public void save(javafx.scene.input.MouseEvent mouseEvent) {
        ...
        dodajTrenera(connection, idTrenera);

        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setHeaderText(null);
        alert.setContentText("Trener został dodany do bazy danych.");
        alert.showAndWait();

        // Czyszczenie pól po dodaniu klienta
        clean(null);
    }
    private void dodajKonto(Connection connection, String idTrenera) {
        String query = "INSERT INTO konta (idKonta, login, haslo, typKonta) VALUES (?, ?, ?, ?)";

        try {
            PreparedStatement statement = connection.prepareStatement(query);
            statement.setString(1, idTrenera);
            statement.setString(2, loginField.getText());
            statement.setString(3, hasloField.getText());
            statement.setInt(4, TypKonta.TRENER.getWartosc());
    ...
    private void dodajTrenera(Connection connection, String idTrenera) {
        String query = "INSERT INTO trenerzy (id, imie, nazwisko, telefon) VALUES (?, ?, ?, ?)";
