...
public class DodajAdminaKontroler {
        ...
        public void save(javafx.scene.input.MouseEvent mouseEvent) {
            ...
            alert.setContentText("Admin został dodany do bazy danych.");
            alert.showAndWait();

            // Czyszczenie pól po dodaniu
            clean(null);
        }
        private void dodajKonto(Connection connection, String idAdmina) {
            String query = "INSERT INTO konta (idKonta, login, haslo, typKonta) VALUES (?, ?, ?, ?)";

            try {
                PreparedStatement statement = connection.prepareStatement(query);
                statement.setString(1, idAdmina);
                statement.setString(2, loginField.getText());
                statement.setString(3, hasloField.getText());
                statement.setInt(4, TypKonta.ADMINISTRATOR.getWartosc());
        ...
        private void dodajKasjera(Connection connection, String idAdmina) {
            String query = "INSERT INTO administrator (id, imie, nazwisko, telefon) VALUES (?, ?, ?, ?)";
